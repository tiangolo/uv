...
pub enum ValidationError {
    ...
    #[error("Can't infer content type because `{}` does not have an extension. Please use a support extension (`.md`, `.rst`, `.txt`) or set the content type manually.", _0.display())]
    ...
    pub(crate) async fn to_metadata(&self, root: &Path) -> Result<Metadata23, Error> {
        ...
        let (license, license_expression, license_files) =
            if let Some(license_globs) = &self.project.license_files {
                ...
                let license = match &self.project.license {
                    None => None,
                    Some(License::Spdx(_)) => {
                        return Err(ValidationError::MixedLicenseGenerations.into())
                    }
                    ...
                    Some(License::File { file }) => {
                        Some(fs_err::tokio::read_to_string(root.join(file)).await?)
                    }
                };
                (license, None, Vec::new())
            };
